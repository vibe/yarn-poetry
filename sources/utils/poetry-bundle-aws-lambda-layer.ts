...
export default async (project: PoetryProject) => {
    ...
    await copy(`${project.path}/${project.projectModuleName}`, `${project.path}/dist/python`)
    var { stderr, stdout } = await exec(`poetry export -f requirements.txt > ${project.path}/dist/python/requirements.txt --without-hashes`, { cwd: project.path })
    ...
    let requirements = await readFile(`${project.path}/dist/python/requirements.txt`, { encoding: 'utf-8' })
    requirements = requirements.replace(/@ \//g, '@ file:///')
    await writeFile(`${project.path}/dist/python/requirements.txt`, requirements, 'utf8')
    ...
    if(hasEnvPackages) {
        ...
        await copy(`${project.path}/.venv/lib/python3.8/site-packages/`, `${project.path}/dist/python/`)
        return 0
    } else {
    ...
    const {code} = await execUtils.pipevp('poetry', ['run', 'pip', 'install', '-r', 'requirements.txt', '-t', '.',], {
        cwd: npath.toPortablePath(`${project.path}/dist/python`),
